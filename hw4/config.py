...
class Config(object):
        ...
        for config in configs[1:]:
            for key in config.keys():
                ...
                if (not isinstance(current_value, Config) or
                    ...
                    merged_config.set(
                            key, cls.merge([current_value, merging_value]))
        return merged_config
    ...
    def set(self, key, value):
        """Sets value to be associated with key, replacing prior value if it
        exists.
        ...
        if current_value is not None and not isinstance(current_value, Config):
            raise ValueError((
                f"Trying to perform nested set with key: {key}, "
                f"but value associated with {nested_keys[0]} is not nested."))
