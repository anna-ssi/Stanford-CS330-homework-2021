...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            num_way = len(unique_labels)
            num_support = len(labels_support) // num_way

            support_embeddings = self._network(images_support).detach()
            query_embeddings = self._network(images_query)

            # Computing the prototypes
            mean_embeddings = support_embeddings.reshape(
                num_way, num_support, -1).mean(dim=1).to(DEVICE)

            # Finding the loss
            query_dists = util.euclidean_metric(
                query_embeddings, mean_embeddings)
            loss = F.cross_entropy(query_dists, labels_query)

            # Support set accuracy
            support_dists = util.euclidean_metric(
                support_embeddings, mean_embeddings)
            y_support = support_dists.softmax(dim=1)
            accuracy_support = util.score(y_support, labels_support)

            # Query set accuracy
            y_query = query_dists.softmax(dim=1)
